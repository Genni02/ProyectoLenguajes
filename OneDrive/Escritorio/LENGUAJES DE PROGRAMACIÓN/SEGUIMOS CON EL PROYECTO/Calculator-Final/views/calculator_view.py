...
class CalculatorView:
    ...
    def create_results_area(self):
        ...
        RoundedButton(
            ...
            text="👣 PASO A PASO",
            radius=15,
            btnbackground=self.colors['primary'],
            btnforeground='white',
            width=132,
            height=30,
            clicked=self.go_to_notebook_view
        ).pack(side=tk.RIGHT, padx=(5, 0))
    ...
    def display_detailed_result(self, expression, result_data):
        """Muestra solo el resultado final (sin pasos)"""
        self.result_text.config(state=tk.NORMAL)
        self.result_text.delete("1.0", tk.END)
        output = f"{result_data['result']}\n"
    ...
    def go_to_notebook_view(self):
        """Cambia a la vista NotebookView y pasa solo la expresión actual"""
        if hasattr(self, 'current_calculation') and self.current_calculation:
            # Pasar solo la expresión como string
            expression = self.current_calculation['expression']
            self.app.views['notebook'].show(expression)
            self.app.show_view('notebook')
        else:
            self.show_notification("No hay cálculo para mostrar paso a paso")